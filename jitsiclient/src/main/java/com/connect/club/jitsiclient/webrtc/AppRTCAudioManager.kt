...
import com.connect.club.jitsiclient.webrtc.AppRTCBluetoothManager.Companion.create
import com.connect.club.jitsiclient.webrtc.AppRTCProximitySensor.Companion.create
...
class AppRTCAudioManager private constructor(context: Context) {
    ...
    private inner class WiredHeadsetReceiver : BroadcastReceiver() {
        override fun onReceive(
            ...
            val state =
                intent.getIntExtra("state",
                    STATE_UNPLUGGED
                )
            val microphone =
                intent.getIntExtra("microphone",
                    HAS_NO_MIC
                )
            val name = intent.getStringExtra("name")
            Log.d(
                TAG,
                "WiredHeadsetReceiver.onReceive" + WebRtcAudioUtils.getThreadInfo() + ": "
                        + "a=" + intent.action + ", s="
    ...
    fun start(audioManagerEvents: AudioManagerEvents?) {
        ...
        if (result == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
            Log.d(
                TAG,
                "Audio focus request granted for VOICE_CALL streams"
            )
        } else {
    ...
    fun stop() {
        ...
        amState =
            AudioManagerState.UNINITIALIZED
    ...
    fun setDefaultAudioDevice(defaultDevice: AudioDevice?) {
        ...
        when (defaultDevice) {
            ...
            AudioDevice.EARPIECE -> defaultAudioDevice = if (hasEarpiece()) {
                ...
                TAG,
                "Invalid default audio device selection"
            )
        }
        Log.d(
            TAG,
            "setDefaultAudioDevice(device=$defaultAudioDevice)"
        )
        updateAudioDeviceState()
    }
    ...
    private fun hasEarpiece(): Boolean {
        return apprtcContext.packageManager
    ...
    private fun hasWiredHeadset(): Boolean {
        ...
        for (device in devices) {
            ...
            if (type == AudioDeviceInfo.TYPE_WIRED_HEADSET) {
                Log.d(
                    TAG,
                    "hasWiredHeadset: found wired headset"
                )
                return true
            } else if (type == AudioDeviceInfo.TYPE_USB_DEVICE) {
                Log.d(
                    TAG,
                    "hasWiredHeadset: found USB audio device"
                )
                return true
            }
        }
    ...
    init {
        ...
        proximitySensor = create(
            context,
            Runnable { onProximitySensorChangedState() }
        )
        Log.d(
            TAG,
