...
class AppRTCBluetoothManager protected constructor(
    ...
    fun start() {
        ...
        if (!hasPermission(apprtcContext, Manifest.permission.BLUETOOTH)) {
            Log.w(
                TAG,
                "Process (pid=" + Process.myPid() + ") lacks BLUETOOTH permission"
            )
            return
        }
        if (bluetoothState != State.UNINITIALIZED) {
        ...
        if (!audioManager.isBluetoothScoAvailableOffCall) {
            Log.e(
                TAG,
                "Bluetooth SCO audio is not available off call"
            )
            return
        }
    ...
    fun startScoAudio(): Boolean {
        ...
        if (scoConnectionAttempts >= MAX_SCO_CONNECTION_ATTEMPTS) {
            Log.e(
                TAG,
                "BT SCO connection fails - no more attempts"
            )
            return false
        }
        if (bluetoothState != State.HEADSET_AVAILABLE) {
            Log.e(
                TAG,
                "BT SCO connection fails - no headset available"
            )
            return false
        }
    ...
    fun updateDevice() {
        ...
        if (devices.isEmpty()) {
            ...
            Log.d(
                TAG,
                "No connected bluetooth headset"
            )
        } else {
    ...
    private fun bluetoothTimeout() {
        ...
        if (devices.size > 0) {
            ...
            if (bluetoothHeadset!!.isAudioConnected(bluetoothDevice)) {
                Log.d(
                    TAG,
                    "SCO connected with " + bluetoothDevice!!.name
                )
                scoConnected = true
            } else {
                Log.d(
                    TAG,
                    "SCO is not connected with " + bluetoothDevice!!.name
                )
            }
        }
        if (scoConnected) {
            ...
            Log.w(
                TAG,
                "BT failed to connect after timeout"
            )
            stopScoAudio()
        }
        updateAudioDeviceState()
    ...
    companion object {
        ...
        fun create(
            ...
            Log.d(
                TAG,
                "create" + WebRtcAudioUtils.getThreadInfo()
            )
